...
const MisReservas = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://api.curso.spazioserver.online/reservas`)
      .then((response) => response.json())
      .then((data) => {
        const filteredReservations = data.filter(
          (reserva) => reserva.usuarioId === userData.id
        );
        ...
        const placeIds = [
          ...new Set(filteredReservations.map((reserva) => reserva.lugarId)),
        ];
        Promise.all(placeIds.map((id) => fetchPlaceName(id))).then(
          (placeNames) => {
            const placeNamesMap = {};
  ...
  const handleConfirmDelete = () => {
    if (reservationToDelete) {
      ...
      fetch(
        `https://api.curso.spazioserver.online/reservas/${reservationToDelete}`,
        {
          method: "DELETE",
        }
      )
        .then((response) => {
          if (response.ok) {
            ...
            const updatedReservations = userReservations.filter(
              (reserva) => reserva.id !== reservationToDelete
            );
  ...
  const { past, future, todayISOString } =
    groupReservationsByDateAndPlace(userReservations);
      ...
      {Object.keys(past).length > 0 && (
        <>
          <Typography
            ...
            onClick={handleTogglePastReservations}
          >
            Reservas Pasadas
            {showPastReservations ? (
              <ExpandLessIcon sx={{ marginLeft: "5px" }} />
            ) : (
              <ExpandMoreIcon sx={{ marginLeft: "5px" }} />
            )}
          </Typography>
          {showPastReservations &&
            Object.keys(past).map((date) => (
              <Box
                key={date}
                sx={{
                  marginBottom: "20px",
                        ...
                        <TableBody>
                          {past[date][placeId].map((reserva) => (
                            <TableRow key={reserva.id}>
                              <TableCell sx={{ textAlign: "center" }}>
                                {truncateTime(reserva.horaInicio)}
                              </TableCell>
                              <TableCell sx={{ textAlign: "center" }}>
                                {truncateTime(reserva.horaFin)}
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
      ...
      {Object.keys(future).length > 0 && (
        <>
          <Typography
            ...
            onClick={handleToggleFutureReservations}
          >
            Reservas Futuras
      ...
      >
        ...
        <DialogActions>
          ...
          <IconButton
            ...
            sx={{ fontFamily: '"Dosis", sans-serif', color: "red" }}
          >
            Sí, Cancelar
          </IconButton>
        </DialogActions>
